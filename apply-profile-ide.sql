-- Pulse Chat: add collision-safe unique six-digit IDEs and repair the new-user profile trigger.
-- Run this once in the Supabase SQL Editor for project eqrfpmwqsstuwkewuknp.
...
alter table public.profiles alter column ide set not null;
alter table public.profiles drop constraint if exists profiles_ide_format_check;
alter table public.profiles add constraint profiles_ide_format_check check (ide ~ '^[0-9]{6}$');
...
where table_schema = 'public' and table_name = 'profiles' and column_name = 'ide';
